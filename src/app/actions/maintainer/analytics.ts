...
import { type RepoHealthRow, type StaleIssueRow, type ContributorRow, type PromotionEligibleRow } from './types';
import { xpToNextLevel, xpForLevel, MAX_LEVEL } from '@/lib/xp/curve';
...
export async function getPromotionEligible(args: {
  ...
  // Find distinct contributors active in this installation's repos.
  ...
  if (eventsError) {
    return err('query_failed', eventsError.message);
  }

  const userIds = Array.from(new Set((eventRows ?? []).map((r) => r.user_id).filter(Boolean)));
  if (userIds.length === 0) {
    return ok([]);
  }
  ...
  for (const p of profileRows ?? []) {
    ...
    const nextThreshold = xpForLevel(next);
    // Within 10% of next level threshold.
    if (needed <= Math.ceil(nextThreshold * 0.1)) {
      eligible.push({
