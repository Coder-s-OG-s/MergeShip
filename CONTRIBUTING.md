...
| **Difficulty** (one system only)   | `level:beginner`, `level:intermediate`, `level:advanced`, `level:critical`                                  | Use these for claimable work. For ECSoC-only issues, use ECSoC-L1 / ECSoC-L2 / ECSoC-L3 labels instead. |
